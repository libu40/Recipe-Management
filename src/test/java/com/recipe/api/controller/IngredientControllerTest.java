...
class IngredientControllerTest {
    ...
    void shouldFetchAllIngredients() throws Exception {
        String fetchedIngredients = mockMvc.perform(get("/api/ingredients").accept(MediaType.APPLICATION_JSON)).andDo(print()).andReturn().getResponse().getContentAsString();
    ...
    void shouldFetchIngredientById() throws Exception {
        String fetchedIngredient = mockMvc.perform(get("/api/ingredients/2").accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk()).andDo(print()).andReturn().getResponse().getContentAsString();
    ...
    void shouldNotFetchIngredientByInvalidId() throws Exception {
        String fetchedIngredients = mockMvc.perform(get("/api/ingredients/100").accept(MediaType.APPLICATION_JSON)).andExpect(status().isNotFound()).andDo(print()).andReturn().getResponse().getContentAsString();
    ...
    void shouldCreateIngredient() throws Exception {
        ...
        mockMvc.perform(post("/api/ingredients").content(asJsonString(new IngredientDto(faker.food().ingredient(), recipeSet, LocalDateTime.now(), LocalDateTime.now()))).accept(MediaType.APPLICATION_JSON).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isCreated()).andDo(print()).andReturn().getResponse().getContentAsString();
    }

    @Test
    void shouldFetchIngredientsBySortedAndPaginatedWithDefaultValues() throws Exception {
        String fetchedIngredients = mockMvc.perform(get("/api/ingredients/page").accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk()).andDo(print()).andReturn().getResponse().getContentAsString();
        List<IngredientDto> ingredients = List.of(objectMapper.readValue(fetchedIngredients, IngredientDto[].class));
        Assertions.assertNotNull(ingredients);
        Assertions.assertEquals("Flour", ingredients.get(0).getName(), "names are not equal");
        Assertions.assertNotNull(ingredients.get(0).getCreatedAt());
